...
class GBTCheck(TestCase):
    ...
    def test_sm4_ecb(self):
        ...
        for i in range(4):
            ...
            self.assertEqual(restore_block, GBTCheck.MESSAGE[i * 16: (i + 1) * 16])
    ...
    def test_sm4_cbc(self):
        ...
        cipher_text = bytearray()
        for i in range(4):
            in_block = GBTCheck.MESSAGE[i * 16: (i + 1) * 16]
    ...
    def test_sm4_ctr(self):
        sm4 = SM4(GBTCheck.SECRET_KEY)
        ctr = CTR(sm4.encrypt_block, 128, iv=GBTCheck.IV_COUNTER)
        ...
        cipher_text = bytearray()
        for i in range(4):
            cipher_block = ctr.update(GBTCheck.MESSAGE[i * 16: (i + 1) * 16])
            cipher_text.extend(cipher_block)
        ...
        for i in range(4):
            ...
            self.assertEqual(restored_block, GBTCheck.MESSAGE[i * 16: (i + 1) * 16])
